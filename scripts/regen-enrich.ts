...
import { enrichMergedSubgroupSummaries } from "../lib/ai/enrich";
 ...
 *   npm run regen-enrich -- tech:ai-news 2026-05-15
 *
...
async function main() {
  ...
  if (category !== "tech" && category !== "politics") {
  ...
  const t0 = Date.now();
  const summaries = await enrichMergedSubgroupSummaries(missing);
  console.log(
