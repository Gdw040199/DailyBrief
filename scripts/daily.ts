...
async function enrichMergedSubgroup(
  ...
  const t0 = Date.now();
  const summaries = await enrichMergedSubgroupSummaries(toEnrich);
...
async function main() {
  ...
  // Enrich GH Trending, papers, arxiv, politics, and AI news with summaries.
  await enrichGhTrending(articles);
  await enrichTrendingPapers(articles);
  await enrichArxivPapers(articles);
